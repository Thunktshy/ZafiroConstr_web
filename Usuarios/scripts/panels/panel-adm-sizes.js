// Control de panel: Sizes (Tamaños) - CRUD con DataTables, jQuery, modales y toasts
...
function normalizeSize(row) {
  ...
  const id  = row.size_id ?? row.id ?? row.sizeId;
...
function mapSizes(listish) { return toArrayData(listish).map(normalizeSize).filter(Boolean); }
...
const modalEl   = document.getElementById("modalSize");
const modalTit  = document.getElementById("modalSizeTitle");
const formEl    = document.getElementById("sizeForm");
const hidId     = document.getElementById("size_id");
...
function openModal(mode = "create", data = null) {
  currentMode = mode;
  modalTit.textContent = mode === "create" ? "Nuevo Size" : "Modificar Size";
...
function openDeleteModal(item) {
  deleteTarget = item;
  deleteMsg.textContent = `¿Seguro que deseas eliminar el size ${item.size_id} — “${item.nombre}”?`;
...
$(document).on("click", "#tablaSizes tbody .js-modificar", async function() {
  const id = Number(this.dataset.id);
  if (!id) return;
  try {
    const resp = assertOk(await sizesAPI.getById(id));
    const item = mapSizes(resp)[0] || null;
...
$(document).on("click", "#tablaSizes tbody .js-eliminar", function() {
  const id = Number(this.dataset.id);
...
/* =========================
   Validación
   ========================= */
function validateSize({ size_id, nombre }, mode) {
  if (mode === "edit") {
  ...
  return { size_id: size_id ? Number(size_id) : undefined, nombre: n };
}
...
function limpiarTablaPorIdSiCoincide(id) {
  const dt = $("#tablaSizePorId").DataTable();
  const rows = dt ? dt.rows().data().toArray() : [];
  if (rows.length && Number(rows[0]?.size_id) === Number(id)) {
    renderDataTable("#tablaSizePorId", [], columnsSoloDatos);
  }
}
...
btnBuscarId?.addEventListener("click", async () => {
  ...
  try {
    ...
    const item = mapSizes(resp)[0] || null;
    if (item) {
      renderDataTable("#tablaSizePorId", [item], columnsSoloDatos);
    } else {
      renderDataTable("#tablaSizePorId", [], columnsSoloDatos);
    }
